...
export default async function Following({params}: {params: {profileId: string}}) {
  const profileId = params.profileId
  const user = await getOthertUserFollowing(profileId)
  ...
  return (
    <div className="bg-[#232323] h-screen w-full flex flex-col px-2 mt-2 rounded-md overflow-scroll">
      ...
      <div className="py-2">
        {user?.map((follower) => (
          <div key={follower.id}>
            <div className="bg-neutral-900/90 w-full items-center h-24 rounded-md p-2 flex gap-x-2 ">
