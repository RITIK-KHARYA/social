import {
  ...
  getOthertUserFollowers,
  getOthertUserFollowing,
} from "@/app/actions/getCurrentUserFollowers";
...
export default async function Followers({
  ...
  params: { followingId: string; profileId: string };
}) {
  console.log(params.followingId);
  const user = await getOthertUserFollowing(params.followingId);
  return (
    <div className="bg-[#121212] h-screen w-[50%] flex flex-col mt-2 rounded-md overflow-scroll">
      ...
      <div className=" flex flex-col py-2 gap-y-1">
        {user?.map((follower) => (
          <Card
            className="flex items-center bg-neutral-900/60 border-2 border-neutral-900 rounded-md w-full ml-2 h-20 box-shadow-md hover:bg-neutral-900/30 "
            key={follower.id}
          >
