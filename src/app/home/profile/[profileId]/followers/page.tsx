...
export default async function Followers({params}: {params: {profileId: string}}) {
    const profileId = params.profileId
  const user = await getOthertUserFollowers(profileId)
  return (
      ...
      <div className="py-2">
        {user?.map((follower) => (
          <div key={follower.id} className="flex gap-y-2 py-2">
            <div className="bg-neutral-900/90 w-full items-center h-24 rounded-md p-2 flex gap-x-2 py-2 ">
